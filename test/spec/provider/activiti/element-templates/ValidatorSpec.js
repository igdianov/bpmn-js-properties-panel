...
var Validator = require('lib/provider/activiti/element-templates/Validator');
...
describe('element-templates - Validator', function() {
  ...
  it('should reject invalid property', function() {
    ...
    expect(errors(templates)).to.eql([
      ...
      'invalid property.binding type <alsoInvalid>; must be any of { ' +
        'property, activiti:property, activiti:inputParameter, ' +
        'activiti:outputParameter, activiti:in, activiti:out, ' +
        'activiti:in:businessKey, activiti:executionListener, ' +
        'activiti:field ' +
      '}'
    ]);
  ...
  it('should reject invalid bindings', function() {
    ...
    expect(errors(templates)).to.eql([
      ...
      'property.binding <activiti:property> requires name',
      'property.binding <activiti:inputParameter> requires name',
      'property.binding <activiti:outputParameter> requires source',
      'property.binding <activiti:in> requires variables or target',
      'property.binding <activiti:out> requires variables, sourceExpression or source'
    ]);
  ...
  it('should reject invalid types for execution listeners', function() {
    ...
    // then
    expect(errors(templates)).to.eql([
      'invalid property type <String> for activiti:executionListener; must be <Hidden>',
      'invalid property type <Text> for activiti:executionListener; must be <Hidden>',
      'invalid property type <Boolean> for activiti:executionListener; must be <Hidden>',
      'invalid property type <Dropdown> for activiti:executionListener; must be <Hidden>'
    ]);
  ...
  it('should reject missing scope properties', function() {
    ...
    expect(errors(templates)).to.contain(
      'template(id: foo) missing properties=[] in scope <activiti:Connector>'
    );
