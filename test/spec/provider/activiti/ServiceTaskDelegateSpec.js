...
describe('service-task-delegate-properties', function() {
  ...
  it('should fill expression property',
    inject(function(propertiesPanel, selection, elementRegistry) {
      ...
      expect(taskBo.get('activiti:delegateExpression')).to.equal('foo');
    })
  );
  ...
  it('should fill delegate expression property',
    inject(function(propertiesPanel, selection, elementRegistry) {
      ...
      // then
      var taskBo = getBusinessObject(taskShape);
      expect(taskBo.get('activiti:expression')).to.equal('foo');
    })
  );
  ...
  it('should remove all other properties in a mutuable choice',
    inject(function(propertiesPanel, selection, elementRegistry) {
      ...
      var taskBo = getBusinessObject(taskShape);
      expect(taskBo.get('activiti:expression')).to.equal('foo');
      expect(taskBo.get('activiti:class')).to.be.undefined;
      expect(taskBo.get('activiti:delegateExpression')).to.be.undefined;
  ...
  it('should apply an empty string to a property', inject(
    function(propertiesPanel, selection, elementRegistry) {
      ...
      // then
      var taskBo = getBusinessObject(taskShape);
      expect(taskBo.get('activiti:class')).to.equal('');
      expect(taskBo.get('activiti:expression')).to.be.undefined;
      expect(taskBo.get('activiti:delegateExpression')).to.be.undefined;
    }
  ));
  ...
  it('should change implementation type from Expression to Java Class for an element', inject(function(propertiesPanel, selection, elementRegistry) {
    ...
    expect(businessObject.get('activiti:expression')).to.equal(delegateField.value);
    expect(resVarField.value).to.equal('resVar');
    expect(businessObject.get('activiti:resultVariable')).to.equal(resVarField.value);
    ...
    expect(businessObject.get('activiti:expression')).to.be.undefined;
    expect(businessObject.get('activiti:resultVariable')).to.be.undefined;
    expect(businessObject.get('activiti:delegateExpression')).to.be.undefined;
  }));
  ...
  it('should remove delegate value field for an element', inject(function(propertiesPanel, selection, elementRegistry) {
    ...
    expect(businessObject.get('activiti:delegateExpression')).to.be.undefined;
    expect(businessObject.get('activiti:class')).to.be.undefined;
  }));
  ...
  it('should add service task properties without adding undefined DMN or external properties', inject(function(propertiesPanel, selection, elementRegistry) {
    ...
    expect(businessObject.get('activiti:topic')).to.be.undefined;
    expect(businessObject.get('activiti:type')).to.be.undefined;
    ...
    businessObject.$model.toXML(businessObject, { format:true }, function(err, xml) {
      expect(xml).to.contain('activiti:expression="' + delegateField.value + '"');
      expect(xml).not.to.contain('activiti:class');
      expect(xml).not.to.contain('activiti:delegateExpression');
      expect(xml).not.to.contain('activiti:topic');
      expect(xml).not.to.contain('activiti:type');
      expect(xml).not.to.contain('activiti:decisionRef');
      expect(xml).not.to.contain('activiti:decisionRefVersion');
      expect(xml).not.to.contain('activiti:decisionRefBinding');
    });
  }));
